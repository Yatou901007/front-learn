...
jQuery.extend(Boxy, {
    ...
    WRAPPER:    "<div class='showSweetAlert c-pop c-confirm boxy-wrapper'><div class='c-pop-box boxy-inner'></div></div>",
    ...
    tip: function(message, callback, options) {
        ...
        var dialog = Boxy.ask(message, false, callback, options),
            timeout;
...
Boxy.prototype = {
    ...
    show: function() {
        ...
        this.boxy.stop().css({opacity: 1,'z-index':'1347'}).show();
        this.visible = true;
    ...
    hide: function(after) {
        ...
        if (this.options.modal) {
            jQuery(document.body).unbind('keypress.boxy');
            this.modalBlackout.animate({opacity: 0,'z-index':'-1'}, function() {
                jQuery(this).remove();
            });
        }
        this.boxy.addClass('hideSweetAlert').stop().animate({'z-index':-1,opacity:0},200,function() {
            self.visible = false;
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            var self = this;
            var tb = jQuery("<div class='hd title-bar'></div>").html("<h2>" + this.options.title + "</h2>");
            if (this.options.closeable) {
                tb.append(jQuery("<a href='javascript:;' class='close closed'></a>").html(this.options.closeText || "<i class='icon-icon_close'></i>"));
            }
